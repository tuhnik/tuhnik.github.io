...
colbox.width = size * riduvaja
    ...
    for(let i = 0; i < cols.length; i++) {
        ...
        cctx.fillRect(col * size, row * size, size, size)

        if(cols[i].toLowerCase() == color.toLowerCase()) {

            console.log("found")
            var luma = detectLuma(color)

            if(luma > 200) {
                cctx.strokeStyle = "#000000cc";
            } else {
                cctx.strokeStyle = "#FFFFFFcc";
            }
            cctx.lineWidth = 4;
            cctx.strokeRect((col * size) + 2 , (row * size) + 2 , size - 4, size -4);
        }
    }
}

colbox.addEventListener("click", chooseColor)

function chooseColor(evt) {
    var rect = colbox.getBoundingClientRect();
    var x = Math.floor((evt.clientX - rect.left) / size)
    var y = Math.floor((evt.clientY - rect.top) / size)
    var ind = y + (x * (colbox.height / size))
    color = cols[ind]
    drawColbox()
}

function detectLuma(c){
